...
function master_problem(num_P, Γ, K, L, pra_dict)
  ...
  #@constraint(model, c7[i=P], [z[i], t[i]] in SecondOrderCone())
  ...
  while true
    ...
    if !sub_problem(submodel, obj_expr, x)
      ...
      etime = time() - stime
      stats = Dict("Minprob"=>value(T), "Minprob:NSWP"=>objective_value(model), "Minprob:transplants"=>value(y[1] + d_1), "Minprob:time"=>etime)
      return stats
      break
    end
