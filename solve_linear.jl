...
function main()
    ...
    model = load_linear(fid["models/linear/path/$nswp_module"], submodel, A)
    ...
    sol, probs = solution(model)
    fid["stats/objective_value/linear/$nswp_module"] = sol
    ...
    fid["stats/pou/linear/$nswp_module"] = price_of_utility(sol, ideal, nadir)
